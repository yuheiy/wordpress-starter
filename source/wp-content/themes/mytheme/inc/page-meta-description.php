...
// https://github.com/WebDevStudios/wd_s/blob/main/inc/hooks/add-og-tags.php
function add_og_tags()
{
	...
	$default_base_description = get_bloginfo("description")
		...
		: esc_html__("Visit our website to learn more.", "wd_s");
	...
	if (is_category() || is_tag() || is_tax()) {
		...
		$specify = is_category()
			? esc_html__("categorized in", "wd_s")
			: esc_html__("tagged with", "wd_s");
	...
	if (is_search()) {
		...
		$card_long_description = sprintf(
			esc_html__("Search results for %s.", "wd_s"),
			$search_term
		);
		$card_description = $card_long_description;
	}
